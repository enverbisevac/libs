...
// using database/sql. Use it with any PostgreSQL-compatible driver
// (lib/pq, jackc/pgx/v5/stdlib, etc.).
package postgres
...
// using FOR UPDATE SKIP LOCKED so concurrent workers don't collide.
...
func (s *Store) MarkProcessed(ctx context.Context, ids ...string) error {
	...
	query := fmt.Sprintf(
		`UPDATE %s SET status = 'processed', processed_at = now() WHERE id IN (%s)`,
